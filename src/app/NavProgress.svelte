<script lang="ts">
	 ...
	 *
	 * Most navigations arrive in tens of milliseconds, so the bar deliberately
	 * waits before appearing — showing it for every navigation would read as a
	 * flicker, and a flicker on a fast page makes a site feel *less* responsive,
	 * not more. It only shows up when a load is slow enough that a visitor would
	 * otherwise wonder if their click registered.
	 *
	 * Belongs inside AppShell's top bar, which is its containing block.
	 */
...
<style>
	.navprogress {
		/* absolute inside the top bar, sitting on its bottom edge: a loading bar
		   belongs to the chrome it is loading under, not floating above the
		   window — and this way it follows the taller bar on narrow screens
