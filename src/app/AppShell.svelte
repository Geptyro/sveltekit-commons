<script>
	/**
	 * The site shell: a top bar, a sidebar that collapses to an icon rail on
	...
	/* Picking a destination closes the overlay; the docked sidebar stays put.
	   ...
	   and `close` is handed to the nav snippet for a tap on the page you are
	   already on, which navigates nowhere and so fires no hook. */
	const close = () => {
